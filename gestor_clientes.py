...
def nuevo_cliente():
    def guardar_cliente():
        ...
        if not telefono.get():
            messagebox.showerror('Error', 'El télefono es necesario')
            return
        
        if not empresa.get():
            messagebox.showerror('Error', 'La empresa es importante')
            return
    ...
    ltelefono = Label(top, text='Télefono')
...
def eliminar_cliente():
    ...
    respuesta = messagebox.askokcancel("Seguro?", "Estas seguro de querer eliminar al cliente" + cliente[1] + "?")
    if respuesta:
...
tree.heading('Telefono', text="Télefono")
